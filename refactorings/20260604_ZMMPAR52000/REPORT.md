...
- [x] **전체 to-be 8개 INCLUDE 클린본 확보** (메인 `zmmpar52000.abap` 포함, HTML→UTF-8 ABAP, 죽은코드 정리, R 컬럼주석) — `to-be/`는 완전 드롭인 세트 ([to-be/README.md](./to-be/README.md))
...
### 적용 체크리스트 + Rollback
- [ ] SE38/Eclipse 원본 백업(비활성 버전 보관)
- [ ] `to-be/` 7개 INCLUDE를 해당 INCLUDE에 반영 후 **문법 점검(SLIN)·활성화**
- [ ] As-Is와 **동일 입력 → 동일 결과**(인터널테이블 건수/정렬/값, 5개 화면 표시) 비교
- [ ] 한국어 주석 100% 유지 확인 / 날짜 변경이력 보존 확인
- [ ] **Rollback**: `git revert` 또는 `as-is/zmmpar52000/` HTML 원본 복원

### 🔍 자가 점검 (최종)
| 항목 | 결과 |
|---|---|
| 의미 보존 (라이브 코드 미변경, SELECT 결과/정렬 동일) | ✅ |
| SY-SUBRC 흐름 동일 | ✅ |
| 부수 효과 동일 (FM 호출/글로벌 변경 동일) | ✅ |
| 한국어 주석 100% 보존 (의도/날짜 이력 포함) | ✅ |
| BINARY SEARCH↔SORT 안전성 | ✅ (C5 PASS) |
| SAP 표준 위배 | ✅ 없음 |
| 죽은코드 제거 추적성 (로그/복구) | ✅ |

**Exit Code: PASS(0)** — 1차 R+M 리팩토링 완료.

### 산출물
- `to-be/` : 7개 INCLUDE 클린 ABAP (UTF-8) — SE38 반영용
- `CLEANUP_LOG_f01.md`, `CLEANUP_LOG_others.md` : 죽은코드 제거 전량 로그
- `as-is/` : 원본 HTML 보존(무수정)
